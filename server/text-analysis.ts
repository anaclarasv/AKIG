  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `Você é um especialista em análise de qualidade de atendimento via chat. Analise a conversa fornecida e retorne uma avaliação detalhada em JSON.
...
Retorne no formato JSON:
{
  ...
  "sentiment": number (0-1)
        ...
        {
          role: "user",
          content: `Analise esta conversa de chat:\n\n${chatContent}`
        }
      ],
...
function generateFallbackChatAnalysis(chatContent: string): ChatAnalysis {
  const wordCount = chatContent.split(' ').length;
  const responseCount = (chatContent.match(/\[Agent\]|\[Cliente\]/g) || []).length;
  
  return {
    responseTime: Math.min(10, Math.max(1, 10 - Math.floor(wordCount / responseCount / 10))),
    professionalism: 7,
    clarity: 7,
    empathy: 6,
    problemResolution: 7,
